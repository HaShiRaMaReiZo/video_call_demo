import './bootstrap';
import { SfuModels, StreamVideoClient } from '@stream-io/video-client';
...
function setControlActive(button, active) {
    ...
    button.classList.toggle('call-btn--active', active);
}
...
function boot() {
    ...
    const updateControlStates = () => {
        const micActive = Boolean(call?.microphone?.state?.enabled);
        const camActive = Boolean(call?.camera?.state?.enabled);
        setControlActive(btnMic, micActive);
        setControlActive(btnCam, camActive);
    };
    ...
    const releaseBindings = () => {
        ...
        if (typeof unbindRemote === 'function') {
            unbindRemote();
        }
        unbindRemote = null;

        if (typeof unbindViewport === 'function') {
            unbindViewport();
        }
        unbindViewport = null;
    ...
    const bindLocal = (c, participant) => {
        ...
        if (!nextSessionId || !localVideo) {
            return;
        }
        if (nextSessionId === localSessionId) {
            tryPlay(localVideo);
            return;
        }
    ...
    (async () => {
        try {
            const { data } = await window.axios.post(tokenUrl);
            const user = {
                id: data.user.id,
                name: data.user.name,
                type: 'authenticated',
            ...
            c.setPreferredIncomingVideoResolution({ width: 1280, height: 720 });

            if (uiEl) {
                unbindViewport = c.setViewport(uiEl);
            }

            await Promise.all([
                c.microphone.enable().catch(() => {}),
                c.camera.enable().catch(() => {}),
            ]);
            ...
            if (btnMic) {
                btnMic.addEventListener('click', () => {
                    c.microphone.toggle().catch(() => {});
                });
            }

            if (btnCam) {
                btnCam.addEventListener('click', () => {
                    c.camera.toggle().catch(() => {});
                });
            }
        } catch (e) {
            const msg = e?.response?.data?.message || e?.message || 'Could not join the call.';
            showError(errEl, msg);
