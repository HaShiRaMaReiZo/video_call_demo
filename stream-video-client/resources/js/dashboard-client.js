...
function boot() {
    ...
    if (statusEl) {
        statusEl.textContent = ds.streamConfigured ? ds.i18nConnected : ds.i18nNotConfigured;
    }
    if (phoneLabelEl) {
        phoneLabelEl.textContent = ds.i18nPhoneLabel;
    }
    if (videoHelpEl) {
        videoHelpEl.textContent = ds.i18nVideoHelp;
    }
    if (incomingLabelEl) {
        incomingLabelEl.textContent = ds.i18nIncoming;
    }
    if (joinBtnLabel) {
        joinBtnLabel.textContent = ds.i18nJoin;
    }
    if (dismissLabel) {
        dismissLabel.textContent = ds.i18nDismiss;
    }
    ...
    const setJoinUi = () => {
        if (joinStatusEl) {
            joinStatusEl.textContent = incoming ? ds.i18nReady : ds.i18nWaiting;
        }
        if (joinBtn) {
            joinBtn.disabled = !incoming || !ds.streamConfigured;
        }
        if (dismissBtn) {
            if (incoming) {
    ...
    if (!ds.userStreamId || !ds.streamConfigured || !ds.tokenUrl) {
        return;
    }
    ...
    (async () => {
        try {
            const { data } = await window.axios.post(ds.tokenUrl);
            const user = {
                id: data.user.id,
                name: data.user.name,
                type: 'authenticated',
